...
<%@ page import="com.jshop.action.front.CallBackAlipayOrderAction" %>
		...
		if(queryRes.isTenpaySign()&& "0".equals(retcode)){ 
			...
			if("1".equals(trade_mode)){       //即时到账 
				if( "0".equals(trade_state)){
					if("3".equals(attachstrs[1].toString())){
						CallBackAlipayOrderAction cbaction=new CallBackAlipayOrderAction();
						int i=cbaction.CallBackToChangeOrderState(out_trade_no.trim(),"9");
						int j=cbaction.CallBackToChangeOrderPayState(out_trade_no.trim(),"1");
						int k=cbaction.CallBackToChangeOrderShippingState(out_trade_no.trim(),"1");
						response.sendRedirect(basePath+"sendEmailVirtualMovieGoodsinfo.action?orderid="+out_trade_no+"&email="+attachstrs[0].toString());
					}
