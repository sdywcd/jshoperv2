...
public class LogisticsbusinessareaTDaoImpl extends HibernateDaoSupport implements LogisticsbusinessareaTDao {
	...
	public int updateLogisticsbusinessarea(final LogisticsbusinessareaT lba) {
		...
		try {
			final String queryString = "update LogisticsbusinessareaT as lba set lba.logisticsid=:logisticsid,lba.logisticsname=:logisticsname,lba.areaname=:areaname,lba.costway=:costway,lba.normalcost=:normalcost,lba.overcost=:overcost,lba.freecount=:freecount,lba.sendarea=:sendarea,lba.state=:state where lba.logbusareaid=:logbusareaid ";
			this.getHibernateTemplate().execute(new HibernateCallback() {
				...
				public Object doInHibernate(Session session) throws HibernateException, SQLException {
					...
					query.setParameter("areaname", lba.getAreaname());
					...
					query.setParameter("freecount", lba.getFreecount());
