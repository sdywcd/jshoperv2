...
public class JshopMGoodsListAction {
	...
	public ArrayList<HashMap<String, Object>> getGoodsList(String goodsCategoryTid) throws IOException{
		...
		if(Validate.StrNotNull(requestjsonstr)){
			...
			for(int i=0;i<ja.size();i++){
				...
				map.put("pictureurlpath", JshopActivityUtil.BASE_URL+jo.get("pictureurl").toString());
				goodslists.add(map);
			}
		}
		return goodslists;
	 ...
	 * @param context
	 */
	public void setGoodsListSQLite(ArrayList<HashMap<String, Object>> goodslists,Context context){
	...
	public ArrayList<HashMap<String, Object>>  getGoodsListSQLite(Cursor c) throws IOException{
		...
		while(!c.isAfterLast()){
			...
			map.put("pictureurl",getPictureurlImg(JshopActivityUtil.BASE_URL+c.getString(c.getColumnIndex("pictureurl")).toString()));
			goodslists.add(map);
			c.moveToNext();
		}
		return goodslists;
